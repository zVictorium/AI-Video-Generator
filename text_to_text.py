...
MODEL = "o3-mini" # "gpt-4o-mini" # "o3-mini" # "o3-mini-2025-1-31" # "gpt-4o-mini"
...
def generate_random_topic() -> str:
    ...
    try:
        ...
        topic = completion.choices[0].message.parsed.description.strip()
...
def generate_brainstorm_response(topic: str, duration: int) -> Video:
    ...
    try:
        # Calculate the estimated characters amount
        completion = client.beta.chat.completions.parse(
            model=MODEL,
            messages=[
                {
...
def generate_image_scene_response(section: str) -> Video:
    ...
    try:
        ...
        return completion.choices[0].message.parsed.scene
    except Exception as e:
        raise RuntimeError(f"Error generating response: {e}")


def generate_image_response(section: str, type: str) -> Video:
    """Generates an image response based on the provided section text."""
    try:
...
def generate_section_response(
    ...
    try:
        messages = [{"role": "system", "content": read_prompt_file("script_en")}]
...
def generate_script(topic: str, duration: int) -> str:
    ...
    section_time = (
        duration * 2200 // (len(brainstorm.sections) + 1)
